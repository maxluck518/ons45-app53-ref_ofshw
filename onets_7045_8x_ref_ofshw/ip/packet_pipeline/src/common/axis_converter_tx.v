...
(
    ...
    output reg                              tuple_out_flowstream_valid
);
   ...
   integer i, j, k;
	...
	reg tx_pkt_byte_cnt_state;
	reg tx_pkt_byte_cnt_state_next;
	always @(*) begin
		tx_pkt_byte_cnt_vld = 0;
		tx_pkt_byte_cnt  = 0;
		tx_pkt_byte_cnt_state_next = tx_pkt_byte_cnt_state;
		case(tx_pkt_byte_cnt_state)
		0:begin
			if(s_axis_tvalid && s_axis_tready) begin
				tx_pkt_byte_cnt_vld = 1;
				tx_pkt_byte_cnt = s_axis_tuser[11:0];
				tx_pkt_byte_cnt_state_next = 1;
			end
		end
		1:begin
			if(s_axis_tlast) tx_pkt_byte_cnt_state_next = 0;
		end
		endcase
	end
	always @(posedge axi_aclk) begin
		if(!axi_resetn) begin
			tx_pkt_byte_cnt_state <= 0;
		end
		else begin
			tx_pkt_byte_cnt_state <= tx_pkt_byte_cnt_state_next;
		end
	end
    ...
    assign tuple_out_flowstream_data = m_axis_tuser;
